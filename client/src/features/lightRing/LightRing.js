...
export const Light = ({ light, degreesPerLight }) => {
  const rotationInDegrees = (light.index - 1) * degreesPerLight;
  const colour = light.on ? light.colour : "white";
  ...
  return (
    <div
      ...
      style={{
        transform: `rotate(${
          rotationInDegrees - 90 + degreesPerLight
        }deg) translate(calc(50% -  3 * ${LightRadius})) rotate(-${
          rotationInDegrees + 270 + degreesPerLight
        }deg) translate(calc(-1 * ${LightRadius}), calc(-${
          light.index * 2 + 1
        } * ${LightRadius})`,
      }}
    >
      <div
        style={{
          backgroundColor: light.on ? colour : null,
          height: `calc(2 * ${LightRadius})`,
          width: `calc(2 * ${LightRadius})`,
          borderRadius: LightRadius,
          boxShadow: light.on
            ? `${colour} 0 0 calc(1.5 * ${LightRadius}) calc(0.2 * ${LightRadius})`
            : `inset ${colour} 0 0 calc(1 * ${LightRadius})`,
        }}
      />
    </div>
  );
